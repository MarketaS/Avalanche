...
glm_data_lbou_C2 <- adcast_C_daily[stat == "LBOU"]
glm_data_lbou_W_C2 <- glm_data_lbou_C2[CAW == "warm"]
glm_data_lbou_C_C2 <- glm_data_lbou_C2[CAW == "cold"]
...
candi = candi[candi!='Rain_Tw_value'] #eliminate already chosen predictor
nn = length(candi)
new_candi = candi[6:nn]
for (i in 1:length(new_candi)){
  print(i)
  M5[[i]] = glm(dta$event ~  dta$SWE_value + dta$SLd_value +dta$WSavg_value + dta$Rain_Tw_value + dta[[new_candi[i]]] )
}
...
M6 = list()
candi = candi[candi!='Rain_Tw_value3'] #eliminate already chosen predictor

nn = length(candi)
new_candi = candi[6:nn]
for (i in 1:length(new_candi)){
  print(i)
  M6[[i]] = glm(dta$event ~ dta$SWE_value + dta$SLd_value +dta$WSavg_value + dta$Rain_Tw_value  + dta$Rain_Tw_value3 + dta[[new_candi[i]]] )
}
...
g_C2 <- glm(formula = event ~ SWE_value + SLd_value +WSavg_value + Rain_Tw_value + Rain_Tw_value3 + + Tair_value3, data =glm_data_lbou_W_C2, family = binomial)
summary (g_C2)
...
lbou_w_C2 <- varImp(g_C2, scale = TRUE)
...
lbou_w_C2[variable == "Tair_value3", variable:= "Tair 3"]
lbou_w_C2[variable == "Rain_Tw_value", variable:= "P_Tw"]
lbou_w_C2[variable == "Rain_Tw_value3", variable:= "P_Tw 3"]
...
M2 = list()
candi = candi[candi!='SWE_value5']
nn = length(candi)
new_candi = candi[6:nn]
for (i in 1:length(new_candi)){
  print(i)
  M2[[i]] = glm(dta$event ~ dta$SWE_value5 + dta[[new_candi[i]]] )
}
...
for (i in 1:length(new_candi)){
  print(i)
  M3[[i]] = glm(dta$event ~ dta$SWE_value5 + dta$SLd_value + dta[[new_candi[i]]] )#building of already selected variables and others
}
...
M4 = list()
candi = candi[candi!='Rain_Ta_value'] # eliminate already chosen predictor

nn = length(candi)
new_candi = candi[6:nn]
for (i in 1:length(new_candi)){
  print(i)
  M4[[i]] = glm(dta$event ~ dta$SWE_value5 + dta$SLd_value +dta$Rain_Ta_value + dta[[new_candi[i]]] )
}
...
candi = candi[candi!='WSavg_value10'] #eliminate already chosen predictor
#Rain_Ta_value
nn = length(candi)
new_candi = candi[6:nn]
for (i in 1:length(new_candi)){
  print(i)
  M5[[i]] = glm(dta$event ~  dta$SWE_value5 + dta$SLd_value + dta$Rain_Ta_value+dta$WSavg_value10 + dta[[new_candi[i]]] )
}
...
M6 = list()
candi = candi[candi!='P_value'] #eliminate already chosen predictor
...
for (i in 1:length(new_candi)){
  print(i)
  M6[[i]] = glm(dta$event ~ dta$SWE_value5 + dta$SLd_value + dta$Rain_Ta_value+ dta$WSavg_value10  +dta$P_value + dta[[new_candi[i]]] )
}
# best predictors for LOWEST AIC right now 
...
g2 <- glm(formula = event ~ SWE_value5 + SLd_value + Rain_Ta_value + WSavg_value10 + P_value, data =glm_data_lbou_C_C2, family = binomial)
summary (g2)
...
lbou_C_C2[variable == "Rain_Ta_value", variable:= "P_Ta"]
lbou_C_C2[variable == "WSavg_value10", variable:= "Wavg 10"]
lbou_C_C2[variable == "P_value", variable:= "P "]
