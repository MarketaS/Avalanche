...
# loading the meteo hourly and snow (daily- hourly value is copied 24 times) data (0: NAd,1: Ad)                                                                     
...
# loading the meteo, snow daily data, we need to join LBOU with HARR and LUCB with PEC according to best formula (slope and intercept function)
...
# all avalanche dates set to 24:00 - from midnight we calculate 6 days prior the Ad event
...
g_SCE <-glm(event ~ SCE_value + SCE_value24 + SCE_value48 + SCE_value72 + SCE_value96 + SCE_value120 + SCE_value144, data = adcast, family = 'binomial')
summary(g_SCE)
...
#summary(gt_SCE_C)
g_T = glm(event ~  T_value + T_value24 + T_value48 + T_value72 + T_value96 + T_value120 + T_value144, data = adcast_W, family = 'binomial')
summary(g_T)
...
dta2 = dta[year(DATE2) %in% 2004:2011]

# how to make for histogram of all variables
#hist <- ggplot(dta_melt)+
...
for(i in 1:length(vars)){
  ...
  ggsave(filename = paste0("C:/Users/marketa.souckova/Documents/laviny/histograms", vars[i], ".pdf"), width = 200, height = 100, units = "mm")
...
for (i in 1:length(new_candi)){
  print(i)
  M3[[i]] = glm(dta$event ~ dta$SVH_value + dta$SNO_value48 + dta[[new_candi[i]]] )#building of already selected variables and others
}
...
for (i in 1:length(new_candi)){
  print(i)
  M4[[i]] = glm(dta$event ~ dta$SVH_value +  + dta$SNO_value48 + dta$T_value24 + dta[[new_candi[i]]] )
}
...
for (i in 1:length(new_candi)){
  print(i)
  M5[[i]] = glm(dta$event ~ dta$SVH_value +  + dta$SNO_value48 + dta$T_value24 + dta$SRA1H_value120 + dta[[new_candi[i]]] )
}
...
for (i in 1:length(new_candi)){
  print(i)
  M6[[i]] = glm(dta$event ~ dta$SVH_value + dta$SNO_value48 + dta$T_value24 + dta$SRA1H_value120 + dta$SSV1H_value72 + dta[[new_candi[i]]] )
...
# SSV1H_48 was the best parametr I would stop the model here.
