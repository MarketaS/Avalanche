...
aval_melt_total <- readRDS(file = "./lbou_lucb_data_new.rds")
...
saveRDS(aval_melt_abc, "./aval_melt_ab.rds")
aval_melt_abc <- readRDS("./aval_melt_abc.rds")
#Tdiff_NULL
...
ggplot(plot_wet)+
  geom_line(aes(x = PLOT, y = V1, color = stat), alpha = 0.7)+
  geom_smooth(aes(x = PLOT, y = V1, color = stat), method = "lm")+
  facet_wrap(~ var, scales = "free_y") +
  scale_x_reverse(breaks = c(0, 24, 48, 72, 96, 120, 144))+
  theme_bw()+
  theme(legend.position = "bottom")+
  labs(title = paste0("Wet avalanches (count ", num_wet, ")"), y = "", x = "number of hours before the avalanche", color = "station")
...
ggplot(plot_slab)+
  ...
  theme_bw()+
  theme(legend.position = "bottom")+
  labs(title = paste0("Slab avalanches (count ", num_slab, ")"), y = "", x = "number of hours the avalanche", color = "station")
