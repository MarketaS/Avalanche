...
# **General information**
This script provides information about the avalanche predictors triggering the avalanche and the differences between two types of avalanches: wet snow and slab avalanches. The two datasets were used for modeling the predictors using machine learning algorithms to obtain insights into what effects play a major role between two different types of avalanches.
...
box plots of all the variables are the key to understand the statistical distribution of data. We checked the differences between the two groups we would like to predict.
...
The differences between both types of avalanches are comparable due to better-balanced data input. More data would be a beneficial factor in better understanding the avalanche triggers' key characteristics.
...
set.seed(1234)
#proportion train 0.75, test set = 0.25☻
...
Synthetics data can be used for evaluation of the DT tree making this technique optimal solution for using DT. The predictability of DT is very high and we can use DT for our data with the power of DT to explain the DT tree process.
...
```
Due to promising results we will implement synthetic data closer to the kerner (datasets with AA and CC suffixes) for the following comparing and modelling sections.
...
Since we train RF we can use for evaluation train / test set. But for final model we can implement the whole dataset due to RF bootstrapping. Therefore we ended up with two models for each dataset. Again, CV would be beneficial. 
