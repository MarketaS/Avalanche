...
# **General information**
This script provides information about the avalanche predictors causing trigerring the avalanche and the differences between two types of avalanches. Two datasets have been used for modelling the predictors using machine learning algorithms to obtain insights what effects play major role between two different types of avalanches.
...
Datasets should not contain any null values. Therefore we excluded those and filtered out observations that start from 1979. The reason for filtering is that we can obtain cleaner datasets. SWE was measured once a week so it could be biased in next sections as well as the variables for 6 days before the avalanche event. Changing variables for modelling is a possible way to get even more accurate results. Next modelling sections aim to obtain reliable predictive models that can be altered by the variables as mentioned before.
...
box plots of all the variables are the key to understand the statistical distribution of data. We checked the differences between the two groups we would like to predict in the next sections.
...
Hclustering was created to obtain information about the distance correlation variables. Screeplots were conducted to sort main clusters by colors. 
...
ggbiplot(prcomp(adcast_C[adcast_C$event == 1, 3:ncol(adcast_C)], scale = TRUE, center = TRUE), 
         ellipse = TRUE,
         alpha = 0.1, 
         varname.size = 5, 
         labels.size = 4, 
...
PCA suggests the main differences in data for both datasets. Both datasets are overpopulated with no avalanche events. The dominance is evident and the avalanche events tend to be hidden in the PCA even though main characteristics of avalanche events in some way persist. The population of avalanche events tends play some role in specific directions but it is hard to distinguish the main deviations for both populations due to low numbers of avalanche events.
The differences between both types of avalanches are comparable due to better balanced data input. More data would be beneficial factor to better understand the key characteristics of the avalanche triggers.
...
```
Random forest classifier has low error rate but cannot predict accurately the avalanche event. 
RF predicts well the non avalanche events but is unable to reflect the avalanches. This is mainly caused because of the unbalanced dataset that we provided for training the algorithm. Bootstrap that is the part of the RF compares in most cases the non avalanche events with each other so the algorithm can predict when there is no avalanche. The avalanche information is hidden and cannot be predicted because it is not in most of the cases the part of the bootstrap selection so the algorithm never classifies the avalanche events (or when it does it is in a few unsignificant cases). Thus we can't predict avalanche events due to the fact of almost never selecting the avalanche events for the decision trees prior the bootstrap. 
For better understanding, we do not compare non avalanche events with avalanche events on decision trees because we select with high probability events with no avalanche events from the bag therefore we have low out of bag error and never predict accurately avalanche event.

Solution to this is to create balanced dataset or weighted decision trees. 

# **Datasets balancing**
Even though we will construct some supervised models in this part of the section. We are in section called datasets balacing that is prior to the final modeling. All the supervised techniques in this chapter are only for demonstration purposes and supervised learning chapter goes after.
...
Upscaled datasets are not eligible for evaluating DT. Test set contains with high probability observations that were used for training the algorithm so we can conclude that this method is not complement for DT. The RF classifier with its bootstrap method can be used to justify the model performance better than DT for upscaling.
To train the RF algorithm we'll use another upscale technique. We have had problems with evaluating our model because of the repeated observations for the testing process. If we generated stratification datasets on event before splitting, we can upscale our datesets and better use them for evaluation. 

```{r}
### RF with upscaled datasets withou stratafication before splitting
randomForest(formula = event ~ ., data = adcast_A_ups)
randomForest(formula = event ~ ., data = adcast_C_ups)
```

Upscaled dataset is better than using the unbalanced datasets for both DT and RF. The error rate decreased and RF does not have the problem to distinguish between the two groups of events and it seems predicting the avalanche event is highly accurate. But the problem related to this is that we have used few observations to train and test the data. We actually used the same test data as for for the training (thanks to the upscale, we are pretty sure the same data went to train and the same for the test) That also means we predict on observations that are known for our model. If we wanted to get accurate results we would have to split the train test set with strata on event, then train and test our results and compare the model performance. The problem with this approach is that we don't have much data for splitting (moreover for dataset_C).
...
Upscaled data before the split tent to be better accurate than unbalanced datasets but worse than data after splitting the data. This is obvious since we did not feed our test data into the training part. We can see that our models underfit. We have to set approach where we can compare both groups of events together. 

## **Synthetic Data Generation**
Anohter way how to deal with unbalanced dataset for our purpose is to use synthetics generation. Instead of replicating and adding the observations from the minority class, it overcome imbalances by generates artificial data.
In regards to synthetic data generation, synthetic minority oversampling technique (SMOTE) can be used. SMOTE algorithm creates artificial data based on feature space (rather than data space) similarities from minority samples. We can also say, it generates a random set of minority class observations to shift the classifier learning bias towards minority class.
...
Thanks to this method we are able to generate data that has similar statistical distribution along the two classes and making our model more robust towards feature data.
...
Synthetics data can be used for evaulation of the DT tree making this technique optimal solution for using DT. The predictability of DT is very high and we can use DT for our data with the power of DT to explain the DT tree process.
...
ggbiplot(both_PCA_t,
  ...
  ggtitle("Both types of avalanche events") +
  scale_x_continuous(limits = c(-2, 2)) + 
  scale_y_continuous(limits = c(-2, 2))
...
Synthetics data can be used for evaulation of the DT tree making this technique optimal solution for using DT. The predictability of DT is very high and we can use DT for our data with the power of DT to explain the DT tree process.
We can see synthetcis data closer to the kernel are much predictable. Our models increased in accuracy.

```{r}
randomForest(formula = event ~ ., data = adcast_synt_AA)
randomForest(formula = event ~ ., data = adcast_synt_CC)
```
Due to promising results we will implement synthetic data closer to the kerner (datasets with AA and CC suffixes) for the next comparing and modelling sections.
...
### Tvorba elementů pro sběr dat z iterací
models <- list()
max_oobs <- c()
var_imp <- list()
pred_comb <- list()
...
for(i in 1:nrow(RF_it)) {
  ...
  # print(models[[i]]) -- informace o modelu
...
### Výsledky iterací dataset_d
max_oobs
var_imp
auc_models
```

First we iterate throught number of trees to see when the OOB decrease. Hence we can establish optimal number of trees used for RF. In our case the number of trees did not have any effect on the OBB so we can use smaller number of trees for our model but since the default number of trees in RF equals to 500. We can leave that.
...
We trained our model to obtain the best hyperparameters for our predictions. The Hyperparameters did not cause significant improvement. But we can be sure our model is reliable and more robust. To improve our model we could use cross validation but it is not necessary.
...
Since we train RF we can use for evaulation train / test set. But for final model we can implement the whole dataset due to RF bootstrapping. Therefore we ended up with two models for each dataset. Again CV would be beneficial. 
...
Our models predict avalanche events well but tent to predict events that were not marked as avalanche bad. In the real scenario we would falsely inform that there is a high probability of the avalanche events. This false information can be thought as warnings for future modelling. To get more accurate results we would need analyse data deeper. 
